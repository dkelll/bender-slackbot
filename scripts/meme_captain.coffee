...
#   hubot Hide yo wife <text> - Antoine Dodsone ftw
#   hubot <text> nobody bats an eye, <text> everyone loses their minds - Joker
#   hubot bronchitis - Ain\'t nobody got time for that!
#
# Author:
#   bobanj, ericjsilva
...
module.exports = (robot) ->
  ...
  robot.respond /(.* nobody bats an eye,?) (.* everyone loses their minds)/i, (msg) ->
    memeGenerator msg, '_U8xnA', msg.match[1], msg.match[2], (url) ->
      msg.send url

  robot.hear /bronchitis/i, (msg) ->
    memeGenerator msg, 'w2atQw', 'ain\'t nobody got', 'time for that!', (url) ->
      msg.send url

  robot.hear /(.*) Hide yo wife (.+)/i, (msg) ->
    memeGenerator msg, 'gnwOMA', 'Hide yo kids, hide yo wife', msg.match[2], (url) ->
      msg.send url
